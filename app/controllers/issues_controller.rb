class IssuesController < ApplicationController
  ...
  def index
		@objects = Issue.order(sort_column + ' ' + sort_direction).page(params[:page])
  end
  ...
  def create
    ...
    respond_to do |format|
      if @issue.save
        format.html { redirect_to issues_path, notice: t('issues.create') }
      else
  ...
  def update
    respond_to do |format|
      if @issue.update(issue_params)
        format.html { redirect_to issues_path, notice: t('issues.update') }
      else
  ...
  def destroy
	...
	def issue_params
		params.require(:issue).permit(:status_id, :report_date, :description, :created_by_id, :notified_by_id, :device_id, :application, :action, :next_action, :detailed_cause, :start_time, :end_time, :closure_date, :detection, :app_status, :category_id, :problem)
	end
